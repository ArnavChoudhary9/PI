...
class CubeLayer(Layer):
    ...
    def OnAttach(self) -> None:
        ...
        mesh: Mesh = self.__AssetManager.Load(AssetManager.AssetType.MeshAsset, ".\\Assets\\Meshes\\TexturedCube.obj")
        mesh.Translate(pyrr.Vector3([ -3.0, 0.0, -7.0 ]))
        self.__Meshes.append(mesh.Name)
    ...
    def OnImGuiRender(self) -> None:
        ...
        with imgui.begin("Settings"):
            ...
            if changed: self.__Light.SetPosition(pyrr.Vector3([ *lightPos ]))

            changed, lightColor = imgui.color_edit3("LightColor", *self.__Light.Diffuse)
            if changed: self.__Light.SetDiffuse(pyrr.Vector3([ *lightColor ]))
        ...
        with imgui.begin("Instructions"):
            imgui.text("Welcome to PI Engine Demo!\nCurrent version: {}\n\n".format(PI_VERSION))
