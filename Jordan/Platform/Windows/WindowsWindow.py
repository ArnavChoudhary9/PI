from ...Core     import JD_VERSION, JD_V_SYNC, JD_CONFIG, JD_DEBUG
...
class WindowsWindow(Window):
    ...
    def __Init(self, props: WindowProperties) -> None:
        ...
        JD_CORE_INFO("Creating Window: {} ({}, {})".format(self.__Data.Title, self.__Data.Width, self.__Data.Height))

        if (not WindowsWindow.GLFWInitialized):
            success: int = glfw.init()
            JD_CORE_ASSERT(success, "Could not intialize GLFW!")
        ...
        if (JD_DEBUG):
            self.__Window = glfw.create_window(props.Width, props.Height, 
                "{} - Jordan v{} - Config: {}".format(self.__Data.Title, JD_VERSION, JD_CONFIG),
                None, None
            )

        else:
            self.__Window = glfw.create_window(props.Width, props.Height, 
                "{} - Jordan v{}".format(self.__Data.Title, JD_VERSION),
                None, None
            )
